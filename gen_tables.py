...
def render_uni_tuple(v: Iterable[int]) -> str:
    return ":".join(uni(cp) for cp in v)


def render_expanded_uni_tuple(v: Iterable[int]) -> tuple[str, ...]:
    return tuple(uni(cp) for cp in v)
...
class AdobeIVDTableRecord(JSONMarshallable):
    ...
    def from_dict(cls, d: dict[Any, Any]):
        return cls(
            ...
            uni_ivs=[tuple(ct) for ct in d["uni_ivs"]],
        )
...
def read_ivd_mappings(fp: TextIO) -> Iterable[AdobeIVDTableRecord]:
...
class _IVSMappings:
    ...
    def lookup_by_single_cp(self, cp: int, table: UniJisTable) -> AdobeIVDTableRecord:
        ...
        if r is None:
            raise ValueError(f"no corresponding IVS mapping found for {uni(cp)}")
...
def do_generate_old_new_table(output: TextIO, input: TextIO, ivd_table: TextIO) -> None:
    ...
    m = _build_ivs_mappings(read_ivd_mappings(ivd_table))
    ...
    for ln, l in enumerate(input, 1):
        ...
        variants: list[AdobeIVDTableRecord] = []
        for i in range(1, len(cols)):
            try:
                if cols[i]:
                    variants.append(m.lookup_by_ivs_svs(tuple(ord(c) for c in cols[i])))
            except ValueError as e:
...
def do_generate_table_ivs_svs_base(output: TextIO, input: TextIO) -> None:
    ...
    for r in read_ivd_mappings(input):
        result.append(
            _IVSSVSBase(
                ...
                base90=uni(r.unijis_90) if r.unijis_90 is not None else None,
                base2004=uni(r.unijis_2004) if r.unijis_2004 is not None else None,
            )
        )
...
RE_GLYPHWIKI_GLYPH_DATA_VERBATIM = re.compile(r"99:0:0:0:0:200:200:(u[0-9a-f]{4,6}\b[^$:]*)$")


def fetch_related_kanji_from_glyphwiki_inner(result: set[str], conn: sqlite3.Connection, name: str) -> None:
    if name in result:
        return
    ...
    try:
        ...
        if m is not None:
            fetch_related_kanji_from_glyphwiki_inner(result, conn, m.group(1))
...
def fetch_related_kanji_from_glyphwiki(conn: sqlite3.Connection, codepoint: int) -> int | None:
    candidates: set[str] = set()
    name = f"u{codepoint:04x}"
    fetch_related_kanji_from_glyphwiki_inner(candidates, conn, name)
...
def do_generate_table_cjk_radicals(output: TextIO, input: pathlib.Path) -> None:
    with sqlite3.connect(input, autocommit=True) as conn:
        json.dump(
            {
...
def add_gen_table_cjk_radicals_subcommand(
    ...
    subparser.add_argument(
        "-i", "--input", type=PathType(exists=True), required=True, help="SQLite3 GlyphWiki dump file"
    )
    subparser.add_argument("-o", "--output", type=argparse.FileType("w"), required=True, help="output file")
    subparser.set_defaults(
        fn=lambda ns: do_generate_table_cjk_radicals(
            output=cast(TextIO, ns.output),
            input=cast(pathlib.Path, ns.input),
        )
    )
    return subparser
